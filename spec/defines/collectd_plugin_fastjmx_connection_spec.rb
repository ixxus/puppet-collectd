...
describe 'collectd::plugin::fastjmx::connection', type: :define do
  let(:facts) do
    {
      osfamily: 'Debian',
      id: 'root',
  ...
  let(:concat_fragment_name) { 'collectd_plugin_fastjmx_conf_foo.example.com' }
  ...
  context 'required params' do
    ...
    it { is_expected.to contain_concat__fragment(concat_fragment_name).with_content(%r{IncludePortInHostname false}) }
    it { is_expected.to contain_concat__fragment(concat_fragment_name).with_content(%r{Synchronous false}) }
    ...
    it { is_expected.to contain_concat__fragment(concat_fragment_name).without_content(%r{TTL}) }
  end
  ...
  context 'include_port_in_hostname' do
    let(:params) do
      default_params.merge(include_port_in_hostname: true,
                           collect: [])
    end

    it { is_expected.to contain_concat__fragment(concat_fragment_name).with_content(%r{IncludePortInHostname true}) }
  end

  context 'synchronous' do
    let(:params) do
      default_params.merge(synchronous: true,
                           collect: [])
    end

    it { is_expected.to contain_concat__fragment(concat_fragment_name).with_content(%r{Synchronous true}) }
  end

  context 'with TTL' do
    let(:params) do
      default_params.merge(ttl: '12345',
                           collect: [])
    end

    it { is_expected.to contain_concat__fragment(concat_fragment_name).with_content(%r{TTL 12345}) }
  end
