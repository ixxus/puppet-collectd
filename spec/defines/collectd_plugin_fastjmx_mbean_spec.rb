...
describe 'collectd::plugin::fastjmx::mbean', type: :define do
  let(:facts) do
    {
      osfamily: 'Debian',
      id: 'root',
  ...
  let(:title) { 'foo' }
  let(:concat_fragment_name) { 'collectd_plugin_fastjmx_conf_foo' }
  ...
  context 'value section plugin_name set' do
    let(:params) do
      default_params.merge(values: [default_values_args.merge('plugin_name' => 'foo')])
    end

    it { is_expected.to contain_concat__fragment(concat_fragment_name).with_content(%r{PluginName "foo"}) }
  end
